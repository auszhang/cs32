...
string Game::takePlayerTurn()
{
	for (;;)
	{
		...
		if (tolower(playerMove[0]) == 'h') // implementation of the h command
		{
			m_arena->history().display(); //if h is pressed, displays the history grid
			cout << "Press enter to continue.";
			cin.ignore(10000, '\n'); //accepts an new line input from the user
			m_arena->display(""); //displays the arena again (without taking a turn)
		}
